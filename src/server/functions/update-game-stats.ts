...
export const updateGameStats = async ():Promise<void> => {
    ...
    links.forEach( async (link) => {
        ...
        const player = players.find(player => `${player.first_name} ${player.last_name}`.toLowerCase() === playerName)

        if(!player) throw new Error(`Player ${playerName} not found in database`)
        ...
        const gamesWithIds = gameStats.map( gameStat => {
            ...
            const game = games.find(game => game.date === dateISO)

            if(!game) throw new Error(`Game not found for date ${gameStat.date}`)
        ...
        const filteredGameStats = gamesWithIds.filter( gameStat => {
            const gameStatInDb = gameStatsInDb.find( gameStatInDb => gameStatInDb.game_id === gameStat.game_id)
